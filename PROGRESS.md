...
- [x] Verified live as Cashier (16/16 automated checks, math exact): walk-in → no discount; VIP member → `-₱11.25` (15% of ₱75.00, source `Membership:Health Plus VIP`, no senior line); VIP + Senior → `-₱13.39` (20% ex-VAT wins, senior line + ID, **no membership line = no stacking**); Family/Chronic at 30% + Senior → `-₱22.50` (membership wins, **no senior line**); membership baseline + tier discount restored to 15% after tests; build 0 warn / 0 err
